...
def main():
    ...
    # unet
    unet = UNet(input_size=args.unet_in_size, input_ch=args.unet_in_ch, T=args.num_train_timesteps, ch=args.unet_ch, ch_mult=args.unet_ch_mult, attn=args.unet_attn, num_res_blocks=args.unet_num_res_blocks, dropout=args.unet_dropout)
    ...
    if args.use_cfg:
        # TODO: class embeder
        class_embedder = ClassEmbedder(embed_dim=args.unet_ch) # TODO: check if this is correct, i have no clue what this is
    ...
    for gen_batch in tqdm(all_images, desc="Processing generated images"):
        gen_batch = torch.stack([transforms.ToTensor()(img) for img in gen_batch]).to(device)
